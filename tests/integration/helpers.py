...
def _deploy_and_configure_minio(
    juju: Juju, bucket_name: str = BUCKET_NAME, s3: str = S3_APP
):
    keys = {
...
def deploy_prometheus(juju: Juju):
    ...
    juju.deploy(
        "prometheus-k8s",
        app=PROMETHEUS_APP,
        # TODO: upgrade to 2/stable once there's something there
...
def deploy_loki(juju: Juju):
    ...
    juju.deploy(
        ...
        app=LOKI_APP,
        # TODO: upgrade to 2/stable once there's something there
        revision=192,  # what's on 1/stable at june 25, 2025.
        channel="1/stable",
        trust=True,
    )
...
def get_tempo_ingestion_endpoint(hostname, protocol: str, tls: bool):
...
def get_tempo_hostname(juju: Juju, ingress: bool) -> str:
    if ingress:
        return get_ingress_proxied_hostname(juju)
    else:
        return get_app_ip_address(juju, TEMPO_APP)
