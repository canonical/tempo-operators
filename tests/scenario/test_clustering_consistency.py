...
from coordinator import MINIMAL_DEPLOYMENT, RECOMMENDED_DEPLOYMENT
...
def tempo_container_ready(context, mock_tempo_workload_ready):
    ...
    return context.run(
        tempo.pebble_ready_event, scenario.State(leader=True, containers=[tempo])
    ).get_container("tempo")


def test_monolithic(context, tempo_container_ready):
    state = scenario.State(leader=True, containers=[tempo_container_ready])
...
def test_scaling_no_s3_inconsistent(context, tempo_container_ready, peers):
    state = scenario.State(leader=True, containers=[tempo_container_ready], relations=[peers])
...
def test_clustered_and_scaled_coordinator_no_s3_inconsistent(
    context, peers, cluster, tempo_container_ready
):
    state = scenario.State(
        leader=True, containers=[tempo_container_ready], relations=[peers, cluster]
    )
...
def test_clustered_coordinator_no_s3_inconsistent(context, tempo_container_ready, cluster):
    state = scenario.State(leader=True, containers=[tempo_container_ready], relations=[cluster])
    state_out = context.run("update-status", state)
    assert_tempo_not_running(state_out)
    assert_status(state_out, unit="blocked")
...
def s3_ready_state(context, tempo_container_ready, peers, s3):
    state = scenario.State(leader=True, containers=[tempo_container_ready], relations=[s3])
...
def test_scaling_with_s3_consistent(context, s3_ready_state, tempo_container_ready, s3, peers):
    state = s3_ready_state.replace(relations=[peers, s3])
...
def test_clustered_scaling_with_s3_consistent(
    context, s3_ready_state, tempo_container_ready, s3, peers, cluster
):
    state = s3_ready_state.replace(relations=[peers, s3, cluster])
...
def test_clustered_with_s3_consistent(context, s3_ready_state, tempo_container_ready, s3, cluster):
    state = s3_ready_state.replace(relations=[s3, cluster])
...
def test_coord_only_scaling_with_s3_consistent(
    context, s3_ready_state, tempo_container_ready, s3, peers
):
    state = s3_ready_state.replace(relations=[peers, s3])
...
def test_coord_only_clustered_scaling_with_s3_inconsistent(
    context, tempo_container_ready, s3, peers, cluster, s3_ready_state
):
    state = s3_ready_state.replace(
        config={"coordinator_runs_workload_when_clustered": False}, relations=[peers, s3, cluster]
    )
...
def test_coord_only_clustered_with_s3_inconsistent(
    context, tempo_container_ready, s3, cluster, s3_ready_state
):
    state = s3_ready_state.replace(
        config={"coordinator_runs_workload_when_clustered": False}, relations=[s3, cluster]
    )
        ...
        c = template.replace(
            ...
            remote_units_data={
                i: TempoClusterRequirerUnitData(
                    juju_topology=JujuTopology(model="model-name", unit=str(i)),
                    address=f"0.0.0.{i}",
                ).dump()
                for i in range(n)
            },
